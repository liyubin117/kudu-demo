...
public class DateUtil {
	...
	public static SimpleDateFormat num_date_formatter = new SimpleDateFormat("yyyyMMdd");
	 ...
	 * @return
	 */
	public static String getNumStringDateShort(Date date) {
		if(date==null){
			date = new Date();
		}
		String dateString = num_date_formatter.format(date);
		return dateString;
	}
	...
	/**
	 * 转换时区函数，传入原来的时区from_zone及对应的时间ds, 转换为to_zone时区的时间
	 * @param ds yyyy-MM-dd HH:mm:ss
	 * @param from_zone 8
	 * @param to_zone 0
	 * @return 转换后的时间字符串 yyyy-MM-dd HH:mm:ss
	 */
	public static String strToStrWithTimeZone(String ds, int from_zone, int to_zone){
		long from_ts = strToDateSql(ds).getTime();
		long to_ts = from_ts - (from_zone-to_zone)*3600*1000;
		return formatter2.format( new Date(to_ts) );
	}
	...
	/**
	 *
	 * @Description: 输入一个整数类型的字符串,然后转换成时分秒的形式输出 例如：输入568 返回结果为：00:09:28 输入null或者“”
	 *               返回结果为:00:00:00
	 * @param @param
	 *            value
	 * @param @return
